...
namespace DFC.App.JobProfile.MessageFunctionApp.Extensions
{
    public static class ServiceCollectionExtensions
    {
        public static IServiceCollection AddRefreshPolicies(this IServiceCollection services, IPolicyRegistry<string> policyRegistry, string keyPrefix, PolicyOptions policyOptions)
        {
            policyRegistry.Add(
        ...
        public static IServiceCollection AddPolicies(this IServiceCollection services, IPolicyRegistry<string> policyRegistry, string keyPrefix, PolicyOptions policyOptions)
        {
            policyRegistry.Add(
                $"{keyPrefix}_{nameof(PolicyOptions.HttpRetry)}",
                HttpPolicyExtensions
                    ...
                    .WaitAndRetryForeverAsync(retryAttempt => TimeSpan.FromSeconds(Math.Pow(policyOptions.HttpRetry.BackoffPower, retryAttempt))))
                    //.WaitAndRetryAsync(
                    //    policyOptions.HttpRetry.Count,
                    //    retryAttempt => TimeSpan.FromSeconds(Math.Pow(policyOptions.HttpRetry.BackoffPower, retryAttempt))))
                ;
        ...
        public static IServiceCollection AddHttpClient<TClient, TImplementation, TClientOptions>(
                   ...
                   where TClientOptions : JobProfileClientOptions, new() =>
                   services
